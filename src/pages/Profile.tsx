...
const IntegrationCard = ({
  ...
  onConnect: (p: "fal" | "openai", s: IntegrationState, ss: React.Dispatch<React.SetStateAction<IntegrationState>>) => void;
  onDisconnect: (p: "fal" | "openai", ss: React.Dispatch<React.SetStateAction<IntegrationState>>) => void;
}) => (
  <div className="liquid-glass-card-sm p-4 rounded-xl space-y-3">
    ...
    {(!state.connected || state.editing) && (
      <div className="space-y-2">
        <div className="flex gap-2">
          ...
          <Button onClick={() => onConnect(provider, state, setState)} disabled={state.loading || !state.apiKey.trim()} size="sm" className="btn-premium rounded-xl h-9 px-4 text-xs">
            {state.loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "Connect"}
          </Button>
          {state.editing && (
            <Button onClick={() => setState((p) => ({ ...p, editing: false, error: "" }))} variant="outline" size="sm" className="rounded-xl h-9 px-2 border-border/50">
    ...
    {state.connected && !state.editing && (
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setState((p) => ({ ...p, editing: true, apiKey: "" }))} className="text-[10px] text-muted-foreground hover:text-primary transition-colors">Change</button>
          <button onClick={() => onDisconnect(provider, setState)} className="text-[10px] text-muted-foreground hover:text-destructive transition-colors">Disconnect</button>
        </div>
      </div>
    )}
  </div>
...
const CreditsSection = ({ userId }: { userId: string }) => {
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    if (payment === "success" && credits) {
      toast.success(`Pagamento completato! €${credits} verranno aggiunti al tuo saldo.`);
  ...
  const handleConnect = useCallback(
    async (provider: "fal" | "openai", state: IntegrationState, setState: React.Dispatch<React.SetStateAction<IntegrationState>>) => {
      ...
      try {
        const { data, error: fnError } = await supabase.functions.invoke("verify-api-key", { body: { provider, api_key: state.apiKey } });
        if (fnError) throw new Error(fnError.message);
        if (data?.error) throw new Error(data.error);
        if (!data?.valid) { setState((p) => ({ ...p, loading: false, error: "Invalid API key. Please check and try again." })); return; }
        const { error: dbError } = await supabase.from("user_api_keys").upsert({ user_id: user!.id, provider, api_key: state.apiKey, is_valid: true }, { onConflict: "user_id,provider" });
        if (dbError) throw new Error(dbError.message);
        const masked = state.apiKey.slice(0, 8) + "..." + state.apiKey.slice(-4);
        setState((p) => ({ ...p, loading: false, connected: true, apiKey: masked, editing: false, error: "" }));
      } catch (err: any) {
        setState((p) => ({ ...p, loading: false, error: err.message || "Connection failed" }));
      }
    },
    [user]
  );
  ...
  const handleDisconnect = useCallback(
    async (provider: "fal" | "openai", setState: React.Dispatch<React.SetStateAction<IntegrationState>>) => {
  ...
  const handleStartChange = () => { setChangingNumber(true); setStep("phone"); setNewPhone(""); setOtp(""); setError(""); };
  const handleCancelChange = () => { setChangingNumber(false); setStep("idle"); setNewPhone(""); setOtp(""); setError(""); };

  const handleSendOtp = async () => {
    if (!newPhone.trim() || newPhone.length < 8) { setError("Enter a valid WhatsApp number with country code"); return; }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (data?.token_hash && data?.email) {
        const { error: authError } = await supabase.auth.verifyOtp({ token_hash: data.token_hash, type: "magiclink" });
        if (authError) throw new Error(authError.message);
        setStep("success");
        setTimeout(() => { setChangingNumber(false); setStep("idle"); setNewPhone(""); setOtp(""); }, 2000);
      }
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="liquid-glass-card p-8 rounded-2xl space-y-6">
                ...
                <AnimatePresence mode="wait">
                  {!changingNumber && step === "idle" && (
                    <motion.div key="change-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <button onClick={handleStartChange} className="text-xs text-muted-foreground hover:text-primary transition-colors flex items-center gap-1.5 mx-auto">
                  ...
                  {step === "phone" && (
                    <motion.div key="phone-step" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                      <div className="liquid-glass-card-sm p-4 rounded-xl space-y-3">
                        ...
                        <Input value={newPhone} onChange={(e) => setNewPhone(e.target.value)} placeholder="+39 123 456 7890" className="bg-input/50 border-border/50 rounded-xl h-11 text-base" type="tel" onKeyDown={(e) => e.key === "Enter" && handleSendOtp()} />
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button onClick={handleSendOtp} disabled={loading || !newPhone.trim()} className="flex-1 btn-premium rounded-xl h-11">
                          {loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <ArrowRight className="w-4 h-4 mr-2" />} Send Code
                        </Button>
                      </div>
                    </motion.div>
                  )}
                ...
                {/* Logout */}
                <Button onClick={handleLogout} disabled={loggingOut} variant="outline" className="w-full rounded-xl h-11 border-destructive/30 bg-destructive/10 text-destructive hover:bg-destructive/20 hover:border-destructive/50 transition-all">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              <div className="liquid-glass-card p-6 rounded-2xl space-y-4">
                ...
                {user && <CreditsSection userId={user.id} />}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
              <div className="liquid-glass-card p-6 rounded-2xl space-y-4">
                ...
                <IntegrationCard
                  provider="fal" label="fal.ai" description="Image & Video generation" url="https://fal.ai/dashboard/keys"
                  icon={<span className="text-lg font-bold text-primary">f</span>}
                  state={fal} setState={setFal} onConnect={handleConnect} onDisconnect={handleDisconnect}
                />
                <IntegrationCard
                  provider="openai" label="OpenAI" description="AI models & assistants" url="https://platform.openai.com/api-keys"
                  icon={<svg viewBox="0 0 24 24" className="w-5 h-5 text-primary" fill="currentColor"><path d="M22.282 9.821a5.985 5.985 0 0 0-.516-4.91 6.046 6.046 0 0 0-6.51-2.9A6.065 6.065 0 0 0 4.981 4.18a5.985 5.985 0 0 0-3.998 2.9 6.046 6.046 0 0 0 .743 7.097 5.98 5.98 0 0 0 .51 4.911 6.051 6.051 0 0 0 6.515 2.9A5.985 5.985 0 0 0 13.26 24a6.056 6.056 0 0 0 5.772-4.206 5.99 5.99 0 0 0 3.997-2.9 6.056 6.056 0 0 0-.747-7.073zM13.26 22.43a4.476 4.476 0 0 1-2.876-1.04l.141-.081 4.779-2.758a.795.795 0 0 0 .392-.681v-6.737l2.02 1.168a.071.071 0 0 1 .038.052v5.583a4.504 4.504 0 0 1-4.494 4.494zM3.6 18.304a4.47 4.47 0 0 1-.535-3.014l.142.085 4.783 2.759a.771.771 0 0 0 .78 0l5.843-3.369v2.332a.08.08 0 0 1-.033.062L9.74 19.95a4.5 4.5 0 0 1-6.14-1.646zM2.34 7.896a4.485 4.485 0 0 1 2.366-1.973V11.6a.766.766 0 0 0 .388.676l5.815 3.355-2.02 1.168a.076.076 0 0 1-.071 0l-4.83-2.786A4.504 4.504 0 0 1 2.34 7.872zm16.597 3.855l-5.833-3.387L15.119 7.2a.076.076 0 0 1 .071 0l4.83 2.791a4.494 4.494 0 0 1-.676 8.105v-5.678a.79.79 0 0 0-.407-.667zm2.01-3.023l-.141-.085-4.774-2.782a.776.776 0 0 0-.785 0L9.409 9.23V6.897a.066.066 0 0 1 .028-.061l4.83-2.787a4.5 4.5 0 0 1 6.68 4.66zm-12.64 4.135l-2.02-1.164a.08.08 0 0 1-.038-.057V6.075a4.5 4.5 0 0 1 7.375-3.453l-.142.08L8.704 5.46a.795.795 0 0 0-.393.681zm1.097-2.365l2.602-1.5 2.607 1.5v2.999l-2.597 1.5-2.607-1.5z" /></svg>}
                  state={openai} setState={setOpenai} onConnect={handleConnect} onDisconnect={handleDisconnect}
                />
              </div>
            </motion.div>
          </div>
